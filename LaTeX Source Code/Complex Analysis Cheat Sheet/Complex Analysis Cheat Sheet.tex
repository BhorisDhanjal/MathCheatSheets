...
\node [mybox] (box){%
    ...
    \begin{itemize}
        ...
        \item Square root: $\sqrt{a + i b}=\pm \left( \sqrt{\frac{a+\sqrt{a^2+b^2}}{2}} + i \frac{b}{|b|} \sqrt{\frac{-a+\sqrt{a^2+b^2}}{2}}\right)$ for \( b \neq 0 \).
...
\node [mybox] (box){%
    ...
    \begin{itemize}
        \item $\C$ and \( \emptyset \) are open subsets of $\C$.
    ...
    \begin{itemize}
        \item \textbf{Interior: } Let $E \subseteq \C$. The interior of $E$ is defined as, $E\degree$=set of all interior points of $E$, or equivalently, $\cup \{\Omega \mid \Omega \subseteq E \text{ and } \Omega \text{ is open in }\C \}$
        \item \textbf{Closure: } Let $E \subseteq \C$. The closure of $E$ is
        defined as
        \(
        \overline{E} = \bigcap\{F \subseteq \C : E \subseteq F \text{ and } F
        \text{ is closed in } \C\}.
        \)
        \item \textbf{Density: }Let $E \subseteq D$, the closure of $E$ in $D$ is $D$. Then $E$ is called dense in $D$.
      
    \end{itemize}
	    \textbf{Path :} A path in a metric space from a point $x \in X$ to $y \in X$ is a continuous mapping $\gamma: [0,1] \rightarrow X$ s.t. $\gamma(0)=x$ and $\gamma(1)=y$.\\
...
\node [mybox] (box){%
    ...
    \begin{itemize}
        ...
        \item For $z = a+ib \ne 0$, one can define a branch of $\mathrm{Arg}(z)$ via
        a suitable version of $\arctan(b/a)$ (with quadrant corrections).
    \end{itemize}
    \end{minipage}
};  
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    \textbf{Uniform convergence: } Let $\Omega \subseteq \C$ and $f_1, \cdots, f_n: \Omega \rightarrow \C$ be a set of functions on $\Omega.$ We say, $\{f\}_{n \in \N}$ converges uniformly to $f$ if given $\varepsilon>0$, there exists $N\in\N$ such that
    $|f_n(x) - f(x)| < \varepsilon$ for all $x\in\Omega$ and all $n \ge N$.\\
    \textbf{Complex exponential: } For $z \in C$, exp$(z)=\sum_{n=0}^\infty \frac{z^n}{n!}$\\
    \textbf{Trigonometric functions: } For $z\in \C$, $\cos (z)=\frac{e^{iz}+e^{-iz}}{2}$ and $\sin (z)=\frac{e^{iz}-e^{-iz}}{2i}$\\
    \textbf{Hyperbolic trigonometric functions: } For $z \in \C$, $\cosh(z)=\frac{e^{z}+e^{-z}}{2}$ and $\sinh(z)=\frac{e^{z}-e^{-z}}{2}$
    \end{minipage}
};  
...
\node [mybox] (box){%
    ...
    \textbf{Radius of convergence:} For a formal power series $\sum a_n(z-z_0)^n$ the radius of convergence $R \in [0,\infty]$ given by $R = \frac{1}{\limsup_{n\to\infty} |a_n|^{1/n}}.$. Using the ratio test is identical i.e. $R = \lim_{n\to\infty} \left|\frac{a_n}{a_{n+1}}\right|
...
\node [mybox] (box){%
    ...
    \begin{itemize}
        \item $F'(z )=\sum_{n=1}^{\infty}n a_n (z-z_0)^{n-1}$ with same radius of convergence $R.$
        \item $a_n=\frac{F^{(n)}(z_0)}{n!}$
    \end{itemize}
    \textbf{Cauchy product of two power series: } For power series $F(z)=\sum a_n(z-z_0)^n$ and $G(z)=\sum b_n(z-z_0)^n$ with degree of convergence at least $R.$ Then the Cauchy product $F(z)G(z)=\sum c_n(z-z_0)^n$ where $c_n=\sum_{k=0}^n a_k b_{n-k}$ also has degree of convergence at least $R$.
    \end{minipage}
};  
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    For a complex function $f(z)=u(z)+iv(z)$,\\ $f'(z)=\frac{\partial u}{\partial x}+i \frac{\partial v}{\partial x}$ or $f'(z)=-i\frac{\partial u}{\partial y}+\frac{\partial v}{\partial y}$\\
    ...
    A function \( f=u+iv \) with \( u,v \) continuously differentiable is holomorphic iff it satisfies
...
\node [mybox] (box){%
    ...
    $\bullet$ For a harmonic function from $\Omega \subset \C$ to $\R$ there exists a uniquely determined harmonic conjugate from $\Omega$ to $\R$ (up to constants) if \( \Omega  \) is a simply connected domain.\\
    \end{minipage}
};  
...
\node [mybox] (box){%
   ...
   Every Möbius transformation is holomorphic on $\widehat{\C}\setminus\{-d/c\}$,
   with a simple pole at $z = -d/c$ for \( c \neq 0 \).
   
    \begin{itemize}
        \item The set of all mobius transformations is a group under composition.
        \item $S$ forms a bijection with $\widehat{\C}$
    \end{itemize}
    Every Möbius transformation can be written as composition of, 
    \begin{itemize}
...
\node [mybox] (box){%
    ...
    \textbf{Arc length:} Arc length of curve $\gamma$ is $|\gamma| = \sup \sum_{i=0}^n |\gamma(x_{i+1}) - \gamma(x_i)|
...
\node [mybox] (box){%
    ...
    \begin{itemize}
        ...
        \item If $|f(z)| \le M$ on $\gamma$, then
        $\left|\int_\gamma f(z)\, dz\right| \leq M\,|\gamma|$.
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    \textbf{Cauchy-Goursat theorem: } Let $f:\Omega\to\C$ be holomorphic. If a curve $\gamma_0$ is homotopic to a reparametrization of $\gamma_1$ then, the integral of some function $f:\Omega \rightarrow \C$ is homotopy invariant, i.e., $\displaystyle \int_{\gamma_0}f=\int_{\gamma_1}f$\\
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    Let $\Omega$ be an open connected set which is symmetric w.r.t $\R$. Then define the following,
    \begin{itemize}
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    Let $(a_n)_{n\in\Z}$ be a bi-infinite sequence ($n = 0, \pm1, \pm2, \dots$).
    
    \begin{itemize}
        \item A doubly infinite series converges if $\sum_{n=0}^\infty a_n$
        and $\sum_{n=1}^\infty a_{-n}$ both converge.
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    Let $\Omega$ be an open connected subset of $\C$ and let $S \subset \Omega$. Let $f: \Omega \setminus S \rightarrow \C$ be holomorphic on $\Omega.$ We say that $f$ is a meromorphic function on $\Omega$ if, 
    \begin{itemize}
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			\textbf{Residue of a function:} Let $f:\Omega\setminus S \rightarrow\C$ be a holomorphic function, where $ \Omega $ is an open set and $ S $ is a discrete subset of $ \Omega $. Then for $ z_0 \in S $, let $r>0$ be s.t. $\overline{B(z_0,r)}\subseteq \Omega$ and
			$B(z_0,r)\cap S = \{ z_0 \}$. Then in $ B(z_0,r) \setminus \{ z_0 \} $, consider the Laurent series expansion of $ f $ given by $ f(z)=\sum_{n=-\infty}^{\infty} a_n(z-z_0)^n$. We define the residue of $ f $ at $ z_0 $ to be $ \text{Res}(f,z_0)= a_{-1}$.
			\begin{enumerate}
				\item If $ z_0 $ is a removable singularity then Res$ (z_0)=0 $.
				\item If $ z_0$ is a pole of order $m$ then $ (z-z_0)^m f(z)=g(z) $, where $ g(z)\neq0  $ on $ B(z_0,r)\setminus\{z_0\} $then, Res$ (z_0)=\frac{g^{(m-1)}(z_0)}{(m-1)!}.$
			\end{enumerate}
		\end{minipage}
	};  
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			...
			For $\Omega=\C \setminus \{\Re(z) \leq 0\}$ define the standard branch to be 
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			Let $ \D $ denote the open unit disc. Let $ f: \D \rightarrow \D  $ be a holomorphic function s.t. $ f(0)=0 $. Then, \[ |f(z)| \leq |z|, \forall z \in \D, \text{ and } |f'(0)|\leq 1	  \]
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			Let $f$ be entire and set 
			$M(f,r)=\max_{|z|=r}|f(z)|$. The \textbf{order} is
			\[
			\rho(f)=\limsup_{r\to\infty}\frac{\log\log M(f,r)}{\log r}.
			\]
			
			\textbf{Borel--Carath\'eodory.}
			If $f$ is holomorphic on $\{\,|z|\le R\,\}$ and $0<r<R$, then
			\[
			\max_{|z|\le r}|f(z)|\le 
			\frac{2r}{R-r}\sup_{|z|\le R}\Re f(z)
			+\frac{R+r}{R-r}|f(0)|.
			\]
			
			\textbf{Jensen's formula.}
			Let $f$ be holomorphic on $\{\,|z|\le R\,\}$ with $f(0)\neq 0$.
			Let $a_k$ be the zeros of $f$ in $|z|<r<R$, counted with multiplicity.
			Then
			\[
			\log|f(0)|
	...
	%------------ Growth of entire functions Header ---------------------
	\node[fancytitle, right=10pt] at (box.north west) {Growth of entire functions};
\end{tikzpicture}
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			\textbf{Canonical factors.}
			For $p\in\N\cup\{0\}$, the canonical factor of genus $p$ is
			\[
			E_p(z)=(1-z)\exp\!\bigl(z+z^2/2+\cdots+z^p/p\bigr).
			\]
			
			\textbf{Weierstrass factorisation.}
			Let $f$ be entire with nonzero zeros $(a_n)$ (with multiplicity)
			and a zero of order $m$ at $0$.
			Then there exist an entire $g$ and integers $p_n\ge 0$ such that
			\[
			f(z)=z^m e^{g(z)}\prod_{n=1}^{\infty}
			E_{p_n}\!\left(\frac{z}{a_n}\right).
			\]
			
			\emph{Existence version.}
			Conversely, given any discrete set $\{a_n\}\subset\C\setminus\{0\}$
			with $|a_n|\to\infty$ and prescribed multiplicities, one can choose
			$p_n$ and $g$ so that the product above defines an entire function
			with exactly these zeros (but with no control on growth / order).
			
			\textbf{Mittag--Leffler.}
			On a domain $\Omega\subset\C$, let $E\subset\Omega$ be discrete and
			for each $a\in E$ fix a finite principal part
			$p_a(z)=\sum_{k=1}^{N_a} c_{a,k}/(z-a)^k$.
			Then there exists a meromorphic $f$ on $\Omega$ whose principal part
			at each $a\in E$ is $p_a$ and which has no other poles.
		\end{minipage}
	};  
	%------------ Weierstrass & Mittag-Leffler Header ---------------------
	\node[fancytitle, right=10pt] at (box.north west) {Weierstrass and Mittag-Leffler};
\end{tikzpicture}
%------------ Hadamard factorisation & Gamma ---------------
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			\textbf{Hadamard factorisation.}
			Let $f$ be entire of finite order $\rho$ with nonzero zeros $(a_n)$
			and a zero of order $m$ at $0$.
			Let $p$ be the smallest integer with
			$\sum_n |a_n|^{-(p+1)}<\infty$.
			Then
			\[
			f(z)=z^m e^{Q(z)}\prod_{n=1}^{\infty}
			E_p\!\left(\frac{z}{a_n}\right),
			\]
			where $Q$ is a polynomial with $\deg Q\le \rho$.
			
			\textbf{Gamma function as example.}
			The reciprocal $1/\Gamma(z)$ is entire of order $1$ with simple zeros
			at $0,-1,-2,\dots$.
			Hadamard + the functional equation give
			\[
			\frac{1}{\Gamma(z)}
			...
			\[
			\Gamma(z)
			= \frac{e^{-\gamma z}}{z}\prod_{n=1}^{\infty}
			\left(1+\frac{z}{n}\right)^{-1}e^{z/n},
			\]
			where $\gamma$ is the Euler--Mascheroni constant.
			
			\textbf{Euler limit form.}
			For $z\notin\{0,-1,-2,\dots\}$,
			\[
			\Gamma(z)
			= \lim_{m\to\infty}
			\frac{m^z\,m!}{z(z+1)\cdots(z+m)}.
			\]
			
			\textbf{Bohr--Mollerup.}
			On $(0,\infty)$, $\Gamma(x)$ is the unique function such that
			\[
			f(1)=1,\quad f(x+1)=x f(x),\quad f>0,\ f\ \text{log-convex}.
			\]
			
			\textbf{Wielandt.}
			If $f$ is meromorphic on $\C$, satisfies
			$f(z+1)=z f(z)$, and is bounded on a vertical strip
			(e.g. $0<\Re z<1$), then $f=c\,\Gamma(z)$ for some constant $c$.
		\end{minipage}
	};  
	%------------ Hadamard factorisation & Gamma Header ---------------------
	\node[fancytitle, right=10pt] at (box.north west) {Hadamard factorisation and $\Gamma$};
\end{tikzpicture}
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			Let $ \Omega =\{z\in \C: a < \Re(z) < b\} $. Let $ f:\overline{\Omega} \rightarrow \C $, s.t. $ f $ is continuous on $ \overline{\Omega} $ and holomorphic on $ \Omega $. Suppose for some $ z=x+iy $, we have $|f(z)|<B $ and let\\
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			\textbf{Normal family.}
			A family $\mathcal{F}$ of holomorphic (or meromorphic) functions
			on a domain $\Omega$ is \emph{normal} if every sequence in
			$\mathcal{F}$ has a subsequence converging locally uniformly on
			$\Omega$ (to a holomorphic map or to $\infty$ in $\widehat{\C}$).
			
			\textbf{Montel's theorem.}
			If $\mathcal{F}$ is a family of holomorphic functions on $\Omega$
			that is locally bounded (bounded on every compact subset), then
			$\mathcal{F}$ is normal.
			
			\textbf{Fundamental normality test.}
			A family of meromorphic functions on $\Omega$ which omits the same
			three values in $\widehat{\C}$ is normal
			(for holomorphic functions, omitting two fixed values is enough).
		\end{minipage}
	};  
	%------------ Normal families & Montel's theorem Header ---------------------
	\node[fancytitle, right=10pt] at (box.north west) {Normal families \& Montel's theorem};
\end{tikzpicture}
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			...
			If $U\subset\C$ is a non-empty simply connected open set with $U\neq\C$,
			then there exists a biholomorphic map $f:U\to\D$.
			
			\textbf{Uniqueness.}
			Given $z_0\in U$, there is a unique such $f$ with
			$f(z_0)=0$ and $f'(z_0)>0$ (or more generally $f'(z_0)\in\partial\D$).
			
		
		\end{minipage}
	};  
	%------------ Riemann mapping theorem Header ---------------------
	\node[fancytitle, right=10pt] at (box.north west) {Riemann mapping theorem};
\end{tikzpicture}
...
\begin{tikzpicture}
	\node [mybox] (box){%
		\begin{minipage}{0.3\textwidth}
			\begin{itemize}
				\item \textbf{Covering: }There is a universal covering $F:\D\to\C\setminus\{0,1\}$;
				any holomorphic $f:\Omega\to\C\setminus\{0,1\}$ with simply connected $\Omega$
				factors as $f = F\circ g$.
				\item \textbf{Little Picard:} A non-constant entire function omits at most one value in $\C$.
				\item \textbf{Great Picard:} At an essential singularity $z_0$, $f$ assumes every complex
				value, with at most one exception, infinitely often in any neighbourhood of $z_0$.
			\end{itemize}
		\end{minipage}
	};
	%------------ Picard theorems Header ---------------------
	\node[fancytitle, right=10pt] at (box.north west) {Picard theorems};
\end{tikzpicture}
